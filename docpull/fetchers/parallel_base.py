...
class ParallelFetcher(BaseFetcher):
    ...
    def fetch_urls_parallel(self, url_paths: List[Tuple[str, Path]]) -> None:
        """
        Fetch URLs in parallel.

        Args:
            url_paths: List of (url, output_path) tuples
        """
        total = len(url_paths)
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all tasks
            futures = {
                executor.submit(self.process_url_with_metadata, url_data): url_data for url_data in url_paths
            }
